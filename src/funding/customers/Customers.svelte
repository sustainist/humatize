    ...
    functions &&
      httpsCallable(
        ...
        .then((result) => {
          ...
          logger = [
            {
              message: "Your monthly contribution was successfully canceled.",
              type: "info",
            },
            ...logger,
          ];
...
<div style:overflow="auto hidden">
  ...
  <table class="list-table">
    ...
    <tbody>
          ...
          <tr>
            ...
            {#if customer.interval === "one-time"}
              <td>
                <div class="spaced">
                  <span>
                    {formatCurrency(customer.currency).currency}:
                  </span>
                  <b>
                  ...
                  {:else}
                    <span class="inline-options" style="display:inline-flex;gap:0.25rem">
                      <label>
                        <input
                          ...
                          type="checkbox"
                          value={customer.id}
                          checked={!activeSubscriptions.includes(customer.id)}
                          onchange={(e) => {
                    ...
                    {#if activeSubscriptions.includes(customer.id)}
                      <form transition:slide>
                        <button
                          class="btn-primary"
                          style="padding:0.125em 0.5em; font-size:0.8rem;"
                          ...
                          onclick={(e) => {
                            if (
                              ...
                              handleCancelSubscription(customer.id);
                            }
                          }}
                        >
      ...
      {:else if !$user?.email}
        <tr>
          <td colspan="3">To see your contributions please sign in</td>
        </tr>
      {/if}
    </tbody>
  </table>
